#!/usr/bin/env node
/**
 * claude_install_hooks.js — safely merges this skill's Stop/PreCompact hooks into
 * an existing ~/.claude/settings.json (or a project one, with --project),
 * without touching anything else already in that file.
 *
 * Usage:
 *   node scripts/claude_install_hooks.js [--project <path>] [--dry-run]
 *
 * What it does:
 *   1. Copies claude_handoff_hook.js next to this script's install location
 *      (~/.claude/hooks/claude_handoff_hook.js), if not already there.
 ...
 *      entry referencing claude_handoff_hook.js isn't already present, so
...
function main() {
  ...
  const marker = "claude_handoff_hook.js";
  ...
  const scriptSrc = path.join(__dirname, "claude_handoff_hook.js");
  const scriptDest = path.join(hooksDir, "claude_handoff_hook.js");
  ...
  const stopCommand = isWindows
    ? '$env:HANDOFF_TOKEN_THRESHOLD=150000; node "$env:USERPROFILE\\.claude\\hooks\\claude_handoff_hook.js"'
    : "HANDOFF_TOKEN_THRESHOLD=150000 node ~/.claude/hooks/claude_handoff_hook.js";
  const precompactCommand = isWindows
    ? 'node "$env:USERPROFILE\\.claude\\hooks\\claude_handoff_hook.js"'
    : "node ~/.claude/hooks/claude_handoff_hook.js";
  ...
  if ((addedStop || addedPrecompact) && fs.existsSync(scriptSrc)) {
    ...
    console.log(`Copied claude_handoff_hook.js to ${scriptDest}`);
  } else if (addedStop || addedPrecompact) {
    console.log(
      `Warning: ${scriptSrc} not found next to this installer — copy ` +
      `claude_handoff_hook.js to ${scriptDest} manually.`
    );
  }
