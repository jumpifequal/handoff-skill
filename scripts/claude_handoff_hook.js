#!/usr/bin/env node
/**
 * claude_handoff_hook.js — cross-platform fallback safety-net handoff writer for
...
function selfValidate(filePath) {
  ...
  try {
    ...
    if (result.status === 2 && result.stdout) {
      process.stderr.write(`claude_handoff_hook: self-check found issues in ${filePath}:\n${result.stdout}`);
    }
  } catch (e) {
