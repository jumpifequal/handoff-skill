...
not a shipped capability). Installing `handoff_hook.js` in a Cowork
...
The hook logic is a single Node.js file (`handoff_hook.js`), not a shell
...
This copies `handoff_hook.js` to `~/.claude/hooks/`, merges only the
...
1. Copy `scripts/handoff_hook.js` to `~/.claude/hooks/handoff_hook.js`
   (same relative path on Windows: `%USERPROFILE%\.claude\hooks\handoff_hook.js`).
